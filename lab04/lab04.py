...
class ArrayList:
    ...
    def append(self, value):
        ...
        if self.len == len(self.data):
            expanded = ConstrainedList((self.len*2)+1)
            for x in range(self.len):
                expanded[x] = self.data[x]
            self.data = expanded
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        n = self._normalize_idx(idx)
        if n > self.len:
            raise IndexError
        if self.len == len(self.data):
            s = ConstrainedList((self.len*2)+1)
            for x in range(idx):
                s[x] = self.data[x]
            s[idx] = value
